...
const getTrueDigPlan = (digPlan) => {
  ...
  for (let dig of digPlan) {
    let valCode = dig.col.slice(1, dig.col.length-1);
    let dirCode = dig.col.slice(dig.col.length-1);
    let dir = nToDir[dirCode];
    let val = parseInt(valCode, 16);
    truePlan.push({dir: dir, len: val})
  }
  return truePlan;
}
...
const solve = (data) => {
  ...
  const truePlan = getTrueDigPlan(digPlan);
  const coords = digPlanToCoords(truePlan);
  const area = shoelaceArea(coords);
  const boundaryPoints = countBoundaryPoints(truePlan);
