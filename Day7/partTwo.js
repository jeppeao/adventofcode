// https://adventofcode.com/2023/day/7#part2// https://adventofcode.com/2023/day/7#part1
...
const cardPriority = cards.reduce((acc, c, i) => ({...acc, [c]: i}),{})
...
const handType = (hand) => {
  ...
  let jokers = count['J'];
  if (jokers) {
    if (jokers === 5) return 1;
  }
  delete count['J'];
  let max = 0;
  let maxCard = '';
  for (let [card, copies] of Object.entries(count)) {
    if (copies > max) {
      maxCard = card;
      max = copies;
    }
  ...
  count[maxCard] = jokers ? count[maxCard] + jokers : count[maxCard];
